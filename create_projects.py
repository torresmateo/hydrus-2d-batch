...
required_arguments.add_argument('--sample-projects', '--sp',
                                help='a path to the folder in which the CROMO project is stored', required=True)
...
proj_dir = os.path.realpath(os.path.expandvars(os.path.expanduser(args.sample_projects)))
print(proj_dir)
...
supported_variables = ['Bulk_d', 'DisperL', 'DisperT', 'DifW', 'SnkL1']
...
template = {'CROMO': '''Pcp_File_Version=4
...
for e, configuration in enumerate(itertools.product(*[data[k] for k in supported_variables])):
    ...
    print(f'copy /E {os.path.join(proj_dir,"CROMO","PRE")} {config_dir}')
    os.system(f'xcopy /E {os.path.join(proj_dir,"CROMO","PRE")} {config_dir}')
    selector = open(os.path.join(config_dir, 'SELECTOR.IN'), 'w')
    selector.write(template['CROMO'].format(**{x: y for x, y in zip(supported_variables, configuration)}))
    selector.close()
    configuration_details += str({x: y for x, y in zip(supported_variables, configuration)}) + '\n'
